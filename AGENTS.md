...
`scripts/generate.sh` から `codex --search exec -` で起動された日次生成では、
以下の制約を必ず適用する。
...
Humanが対話的にCodexを起動し、調査、テスト、コード修正などの保守作業を
明示的に依頼した場合に限り、その依頼範囲内で保守作業モードを適用できる。

- Codexは保守作業モードを自ら推測して開始してはならない。
- リポジトリの読み取り、必要なシェルコマンドの実行、およびHumanが明示的に許可したファイル変更を行える。
- Scopeの制約は維持し、このプロジェクト内のファイルだけを対象とする。
- Git操作はHumanが明示的に許可した場合だけ行う。
- ニュース生成を依頼されていない保守作業では、Web検索、ニュース収集、Brief生成、再生成を開始してはならない。
